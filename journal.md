...
I finished the front plate, which hides all the ugly screws and PCBs, and makes for a nicer finished product, I probably need polish, and finally make an assembly.
The proccess was striaght forward, I made a rectangle which is an copy of the plate one, I added two screws, since this is cosmetic and doenst hold weight, an hole for screens and meters, and the keyboard, I also made this sit nice and flush by making it lower than the meters.
...
I actually want to show what I finished, which i'm so so so proud, the circle idea was taken from Lex Bailey, shoutout to them, 
![image.png](https://cdn.hackclub.com/019fee35-7a45-7e8a-b86d-f6070f9d24cd/image.png)
I just feel it's so professional omg, it looks like swiss cheese, but looking at the sketch will make it make more sense
![image.png](https://cdn.hackclub.com/019fee36-9658-795e-b904-0997d9a53142/image.png)
As you can see, each hole is a screw, the big hole at the meters houses the thing the meter uses to put the gauge, the bunch of hashtags are variables, why? Those let me adjust stuff that's commonly repeatable, the reason i'm not doing an linear pattern is that I forgot those existed and I was win too deep, so that's why, but I did use it back at the encoder, ye, that's it
...
![image](https://cdn.hackclub.com/019fe3f1-73c0-7ffc-902b-8238f351661a/image.png)
![image](https://cdn.hackclub.com/019fe3f1-a8fd-7419-b70f-b0570b7b3dc6/image.png)
...
Well, you have to make .3-.4mm smaller than the outside diameter and add a chamfer to guide it in!
![image](https://cdn.hackclub.com/019fd83f-9ddb-7a7c-a98c-5dfac4328fad/image.png)

*2. How do I reference PCB design in sketch*
Onshape has a feature called context editing for this! It overlays the other parts in the assembly on top, and allows referencing them in sketches
![image](https://cdn.hackclub.com/019fd840-d537-71c3-bfcb-e0e0c64f02de/image.png)

*3 .How do I install the input board, angling the keys didn't look good so i'm placing them on top + i get to make it lighter?*
Since it's an triangle, only placing the 85C1 would have left no space for the mainboard pcb in the opposite cathetus/side. (cathetus, the smaller sides of the triangle, as opposed to the hypotenuse)

*4. This is huge, 256mm x 63mm x 80??mm, not confirmed yet, i'm worried. This is it, i'm so worried and confused and ahhhhhhhh*
Yeah, I WAS TIRED, anyways, just splitting it into two parts solved our issues, the only issue I have is the left part (pictured as the one doing nothing next to the one holding the PCB) is useless
![image](https://cdn.hackclub.com/019fd838-89da-78c2-a282-05379c08580d/image.png)
...
1. How do I insert heatset insert? 
2. How do I reference my pcb design in my sketch?
3. How do I install the input board, angling the keys didn't look good so i'm placing them on top + i get to make it lighter
4. This is huge, 256mm x 63mm x 80??mm, not confirmed yet, i'm worried.
This is it, i'm so worried and confused and ahhhhhhhh

5 minutes later, I have tought of a solution, previous designs stacked the meters side by side, but i'll do two blocks of 10mm of each, and make the size smaller
![image](https://cdn.hackclub.com/019fc95c-9eb3-7a63-ba49-cc17b5c39ce7/image.png)

Oh yeah, I finished the encoder knob, and I hope this one is more 3d printer friendly
![image](https://cdn.hackclub.com/019fc961-e1ee-7277-a136-af6b45ea2a9e/image.png)
...
Then I started to make the encoder knob, i'm not sure if this printable, and I maybe should consider getting an straight knurling instead, in fact, i'm actually gonna do that right now! Yeah, incredible idea. Anyways, knurling this took me an hour and half, because I had to figure out a bunch of new stuff. i'm not actually so proud of this, i don't know, it just looks evil and violent. Next step is redoing this
![image](https://cdn.hackclub.com/019fbe7e-2d70-7f44-a748-9d24b76c2227/image.png)
...
But then I realized it needs to be in the same parts studio as my input board, so I deleted it with the intention of starting it over again. 
But, for some reason I do not know, I decided that I would put work on making the meters instead, and uhhh, it went well! Up until i spent 3hrs to realize the model I used was bogus, hooray, and I was so close too :((((. Anyways, i'm starting fresh, and i hope i get it all done by tomorrow
![image](https://cdn.hackclub.com/019fba9b-6dd9-7053-b9ae-73b0e8bd3bcd/image.png)
...
![image](https://cdn.hackclub.com/019fb4a6-9710-796a-b549-b71c7f0b2198/image.png)
...
![image](https://cdn.hackclub.com/019fa975-690a-7059-8ffa-42dfaa1eb8b8/image.png)
...
Finally, to wrap it all up, I cleaned up most warnings, there's just one that throws an error when I try to fix it LOL.
![image](https://cdn.hackclub.com/019fa5bb-2f60-7739-82ed-1ceba11854bb/image.png)
...
Anyways, after that, things went okay, I did have an hiccup when getting icons to show on the screen, because the library I initially used was garbage, or so I tought, atleast that's what I tought at the time, but after switching to a different library, I managed to get the notification icon.
Then, implementing the notification UI was a breeze, I had a hiccup because I had to learn linearlayout, but besides, that, it went okay.
Then, I implemented the music player and default screens, while the default screen went okay, the music player, which is where I am now, is a bit hard because I need to figure out how do I show progress, I initially tought of a bar, but i'm thinking of just using a string, tho that would be sad.
During that, I don't know how, but something told me that the previous library for icon (which was superior due to it having more icons) was working and that I was wrong, sure enough, the issue was me not implementing the feature for the icon I wanted to implement
I am now stopping this devlog because it got too big, but that's where i am now.
![image](https://cdn.hackclub.com/019fa4f6-d1a3-7794-8dc3-7552efa3265e/image.png)
...
![image](https://cdn.hackclub.com/019f9622-3189-77b8-813a-7605e6df2bef/image.png)
![image](https://cdn.hackclub.com/019f9624-d573-76c3-933e-90b2bd9d8c41/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8df-8c6b-7e9d-b6d4-4255ab6c13af/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8df-20b6-7261-ac69-e7a19e540cb1/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8de-294a-7cf7-b7dc-9aa30a231aee/image.png)
![image.png](https://cdn.hackclub.com/019fe8de-3887-7ee6-b9c1-71caf6133ece/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8da-27b8-70ad-940a-859c4578db19/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8da-530c-76b7-b4f7-da7d65c83827/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8c0-d2a5-7a9f-b533-5ff87b7641d4/image.png)
![image.png](https://cdn.hackclub.com/019fe8c0-f478-75e7-b7da-21654044d013/image.png)
...
However, I am struggling to find a library handling both cpu temperature and memory and it drove me insane!

I am probably going to have to accept defeat, and instead opt for getting memory and swap from sysinfo rust crate, and temperature from the hwmon or thermalzone files in Linux... 
By the way, yes, I did not work the past few days
![image.png](https://cdn.hackclub.com/019fe8b6-66bb-7c06-9877-4436ba323a06/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8b5-7fa4-7812-87b9-243893416802/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8b5-11bd-70cc-a900-6683a5e5dba4/image.png)
...
However, the other half was just something else. What happened, was that the connectors were all specified as bidirectional pins, so, KiCad freaked out like i just put 120V on an I2C line So, I decided I should try making my own symbols manually
Horrible mistake

Kicad sent my library to outer space and I spent 20 minutes trying to find where did it go. Not only that, but I didnt even need to actually make a new symbol! I was using one connector library, and all I had to do was switch to the other! I hope I can start making the PCB now And if you're curious, the ammeter is our gauge, the thing we are gonna drive. 
![image.png](https://cdn.hackclub.com/019fe8b4-9afc-70a7-8df2-500163af64aa/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8b3-90d6-7081-8a5c-d73f6dd3c007/image.png)
...
![image.png](https://cdn.hackclub.com/019fe8b3-4e79-7ddc-ba3c-88e7fc111780/image.png)
...
![image](https://cdn.hackclub.com/019f104c-be8b-7382-a0b4-336d654c05d8/image.png)
...
Basically, I figured out I was looking for exact precision, when not even the machines are that precise So what did I do then? Well, I still used Kicad's finest grid of 0.001mm, and i got the thru hole aligned with a precision of 0.0005mm, or an micrometer, which is perfect. 
![image.png](https://cdn.hackclub.com/019fe8b1-a965-7637-8169-1e5fb8e0fd97/image.png)
Other stuff
...
![image.png](https://cdn.hackclub.com/019fe8b2-0b11-7489-93e0-e9bae7f540df/image.png)
...
This was an horrible mistake
What I didn't realize, is that for some reason, the PCB adafruit provides is off-center, this is def an issue in my part during some part of the conversion process. Even on the finest grid, it's still of center, just why?]
![image.png](https://cdn.hackclub.com/019fe8b1-5064-7af9-bed0-df491a3ea138/image.png)
